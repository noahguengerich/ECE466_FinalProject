...
)
{
    ...
    {
        R_memcpy((POINTER)block, (POINTER)&randomStruct->output[16 - available],
                 available);
        block += available;
        blockLen -= available;
    ...
    NN_DIGIT c)
{
    ...
    if (t[0] != tRef[0])
        cout << "t[0] = " << t[0] << " --- expected: " << tRef[0] << endl
             << endl;
    if (t[1] != tRef[1])
        cout << "t[1] = " << t[1] << " --- expected: " << tRef[1] << endl
             << endl;
    if (aHigh != aHighRef)
        cout << "aHigh = " << aHigh << " --- expected: " << aHighRef << endl
             << endl;
...
void dh_sw::process_sw()
{
    ...
    if ((status = R_GenerateDHParams(&PARAMS2, KEY_LENGTH, (KEY_LENGTH / 2), &randomStruct)))
    {
    ...
    */
        do
        {
            if ((status = R_SetupDHAgreement(publicValue, privateValue, KEY_LENGTH, params, &randomStruct)))
            {
    ...
    while (keys_exchanged != NUM_KEYEXCHANGE)
    {
        ...
        if ((status = R_GenerateDHParams(&PARAMS2, KEY_LENGTH, (KEY_LENGTH / 2), &randomStruct2)))
        {
            printf("ERROR: Code 0x%04x while generating parameters\n", status);
            break;
        }
        ...
        */
            do
            {
                if ((status = R_SetupDHAgreement(publicValue, privateValue, KEY_LENGTH, params, &randomStruct2)))
                {
                    printf("ERROR: Code 0x%04x while setting up key agreement\n", status);
                    break;
                }
         ...
         */
            do
            {
                if ((status = R_ComputeDHAgreedKey(agreedKey, gen_otherPublicValue, gen_privateValue, KEY_LENGTH, params)))
                {
